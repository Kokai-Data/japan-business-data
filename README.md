...
## Migration: agent plugins consolidated into kokai-data (v0.4.0)

v0.4.0 から、7 本の agent plugins (`proposal-prep-jp` / `subsidy-fit-jp` / `due-diligence-jp` / `competitor-brief-jp` / `subsidy-landscape-jp` / `legal-research-jp` / `procurement-discovery-jp`) は **`kokai-data` に統合**されました。機能はすべて `kokai-data` 1 本に含まれます（agents / slash commands / skills / MCP 接続）。

If you previously installed any of the 7 agent plugins:

1. Uninstall the agent plugin(s) (`claude plugin uninstall proposal-prep-jp@kokai` etc., or remove in the GUI plugin list).
2. Install / update the single plugin: `claude plugin install kokai-data@kokai`.
3. Your slash commands (`/提案準備`, `/企業調査`, `/proposal-prep`, etc.) and agents work the same — the agent name prefix changes from `proposal-prep-jp:` to `kokai-data:` when invoked as a subagent type.

Why: 8 entries in the marketplace list confused first-time users ("which do I install?"). One plugin = one instruction: **install `kokai-data`, get everything**.
...
|---|---|---|
| **Plugin (all-in-one)** | **1** | `kokai-data` — MCP connector + 19 skills + 16 slash commands (JA/EN) + **7 agents** (proposal-prep / subsidy-fit / due-diligence / competitor-brief / subsidy-landscape / legal-research / procurement-discovery) |
| Managed-agent cookbook (Claude Managed Agents deployment template) | 1 | kokai-due-diligence-jp |

> v0.4.0 で **agent plugins 7 本を `kokai-data` に統合**しました（一覧に並ぶ plugin は 1 本だけ。install も 1 コマンドだけ）。旧 agent plugin をお使いの方は下の [Migration](#migration-agent-plugins-consolidated-into-kokai-data-v040) を参照。

## Agents (7 named templates — `kokai-data` に同梱)
...
├── plugins/
│   └── vertical-plugins/
│       └── kokai-data/                    # ← the ONE plugin (v0.3.0 で agent plugins を統合)
│           ├── agents/                    # 7 agents (proposal-prep / due-diligence / subsidy-fit / competitor-brief / subsidy-landscape / legal-research / procurement-discovery)
│           ├── commands/                  # 16 slash commands (日本語 8 + English alias 8)
...
claude plugin install kokai-data@kokai   # これ 1 本で全部入り (MCP + slash + skills + agents)
```
...
5. After the marketplace is added, install **`kokai-data`** — the single all-in-one plugin (MCP + slash commands + skills + 7 agents). Nothing else to choose.
